...
# On a fresh fedora 39 install there appear to be subinterpreter problems with
# python 3.12.0 (and these show up as a failure in regina's test suite).
# These problems are not present in fedora 40, which ships with python 3.12.2.
# So: to avoid these problems we require python >= 3.12.2 (which users should
# already have nowadays via the fedora 39 updates repo).

Requires: mimehandler(application/pdf)
Requires: python3 >= 3.12.2
Conflicts: regina
